...
public class Notes_fragment extends android.support.v4.app.Fragment{
                ...
                public boolean search(String s) {
                    ...
                    if (s.length() == 0) {
                        ...
                        while (i < myadapter.getItemCount() && i<c.length) {
                            ...
                            if(c[i].getName().replace("»","").replace("%20"," ").contains(s))
                            {
                                change=1;
                                remlist.add(c[i]);
    ...
    public void onStart() {
        ...
        int Images;
        Localdb db = new Localdb(getActivity().getApplicationContext(), Find.dept(rn), null, 1);
        c = db.outdownload("select * from downloads where type='notes'");
        final List<String> notelist = new ArrayList<>();
        int i = 0;
        if (c[i].getPath() == null)
            notelist.add("Nothing Found");
        while (c[i].getPath() != null) {
            Images=getImage(c[i].getName());
            //Ends Here
            notelist.add(c[i].getName());
            myadapter.add(myadapter.getItemCount(),
                    c[i].getName().replace("%20"," "), Images);
            i++;
        }


        noteadapter = new ArrayAdapter<String>(getActivity().getApplicationContext(), android.R.layout.simple_list_item_1, notelist);
    ...
    public void onActivityCreated(Bundle savedInstanceState) {
        ...
        myrecycler = (RecyclerView) getActivity().findViewById(R.id.notelist);
